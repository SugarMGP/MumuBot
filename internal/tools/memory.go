...
func saveMemoryFunc(ctx context.Context, input *SaveMemoryInput) (*SaveMemoryOutput, error) {
	...
	result, err := tc.MemoryMgr.CommitKnowledgeBatch(ctx, batch)
	if err != nil {
		return nil, classifyMemoryToolError(err)
	}
	tc.MarkActed()
	message := "已保存并启用"
	for _, status := range result.ItemStatuses {
		if status == "archived" {
			message = "已补充依据；已有记忆仍为归档状态，未重新启用"
		}
	}
	return &SaveMemoryOutput{Success: true, Message: message}, nil
}
