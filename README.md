...
<p align="center">
  ...
  <a href="https://github.com/SugarMGP/MumuBot/blob/main/LICENSE"><img alt="License" src="https://img.shields.io/badge/license-GPL--3.0-blue?style=flat"></a>
...
- 🧩 **丰富工具集** — 内置工具覆盖发言、沉默、统一记忆检索、工作便签、戳一戳、贴表情、发表情包、查群信息和网页浏览，并可通过 MCP 扩展
- 📝 **长期记忆** — 基于 PostgreSQL、pgvector 与 pg_trgm 统一检索事实、经历、偏好、群术语和表达方式；关联与完整消息证据可追溯
- 👤 **群友画像** — 聚合有消息证据的成员知识与参与讨论，后台记忆 Agent 按需调查历史语境
- 🎭 **情绪系统** — 心情、精力、社交意愿三维情绪状态随对话自然变化，影响说话方式和活跃度
- 👀 **多模态理解** — 支持视觉模型识别图片和视频内容
- 🖼️ **表情包系统** — 自动收集群内表情包，由智能体自行决策使用
- 📖 **统一整理** — 一个后台 Memory Agent 批量完成话题归属、摘要和知识维护；按群冷却、轮转调度，共享请求间隔，避免重复调用模型
