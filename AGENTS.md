...
- `think` 必须基于固定消息快照，思考期间新到消息留给后续正常调度，不主动追加本轮或触发 rerun；`speak` 保持一至五条消息，每条可独立设置 `reply_to` 和 `mentions`。
- 同群串行队列只负责事件解析、消息去重、原始消息入库、撤回状态和缓冲区顺序；标已读、回复补全、合并转发拉取、视觉识别、成员画像更新等慢操作必须在消息完成核心入队后异步执行，不能阻塞同群后续事件。
- 群聊展示中的机器人提及必须直接用 Adapter 的运行时 `self_id` 识别并渲染为“@{persona.name}(你)”；普通成员优先使用 OneBot 事件自带称呼，缺失时只能在同群串行队列之外的补全或主动发言路径查询群名片/昵称，禁止在串行队列中调用成员信息接口。
- 慢操作只能按消息 ID 条件更新对应数据库记录和缓冲项，不得重新追加消息、改变既有顺序或覆盖撤回等较新状态；补全使用新消息对象替换当前版本，不原地修改已经进入思考快照的对象。
- 消息触发的思考防抖从当前进程收到 OneBot 事件的时间开始计算；核心入库和缓冲更新完成后只等待剩余窗口，异步补全不得延后或重置该窗口。思考仍使用开始时取得的固定消息快照。
